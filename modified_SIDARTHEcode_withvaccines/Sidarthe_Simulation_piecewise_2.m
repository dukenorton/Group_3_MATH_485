...
%
%This code is modified by Carl Ingebretsen to implement piecewise
%vacciantions
...
for i=2:length(t)
    ...
    %Do this by changing the vaccination rate phi and reinfection rate
    %rein.
    if (i>5/step)
        phi=0.00001;
        %rein=0.000001;
    ...
    %x=x+B*x*step;
    %in row one decide between [phi*x(1), phi*x(9) or just phi]
...
title('Epidemic Evolution with Piecewise Vaccination')
xlabel('Time (days)')
...
legend({'Cumulative Infected','Current Total Infected', 'Recovered', 'Vaccinated'},'Location','northwest')
