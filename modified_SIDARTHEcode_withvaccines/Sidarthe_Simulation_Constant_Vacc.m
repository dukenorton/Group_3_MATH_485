...
for i=2:length(t)
    ...
    %This is the matrix that evolves the system
    % Compute the system evolution
    %ADDED A NEW zero to the end of each row and added a tenth row with vaccination 
...
% FIGURES
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%print("Final value of infected is: ", Infetti_reali(length(t)))

% Add a V term to the first two graphs
figure
plot(t,Infetti_reali,'b',t,I+D+A+R+T,'r',t,H,'m',t,E,'k',t,V,'black--','LineWidth',2.5)%added V
hold on
%plot(t,D+R+T+E+H_diagnosticati,'--b',t,D+R+T,'--r',t,H_diagnosticati,'--g','LineWidth',2.5)
xlim([t(1) t(end)])
ylim([0 0.015])
%title('Actual vs. Diagnosed Epidemic Evolution with Constant Vaccination')
title('Epidemic Evolution with Constant Vaccination')
xlabel('Time (days)')
...
legend({'Cumulative Infected','Current Total Infected', 'Recovered', 'Deaths', 'Vaccinated'},'Location','northwest')
fontsize(legend,18,"points")
grid
